...
def upgrade() -> None:
    ...
    op.execute("SELECT setval('activities_id_seq', (SELECT COALESCE(MAX(id), 0) + 1 FROM activities))")
    op.execute("SELECT setval('organizations_id_seq', (SELECT COALESCE(MAX(id), 0) + 1 FROM organizations))")
    op.execute("SELECT setval('buildings_id_seq', (SELECT COALESCE(MAX(id), 0) + 1 FROM buildings))")
